...
function registerReturnShipment(o, returnLocker) {
  ...
  var sender = {
    // Vardas + užsakymo nr (kad matytųsi, iš ko ir kurio užsakymo grąžinimas).
    personName: nameWithOrder(o['Klientas'], orderNo),
    altName: nameWithOrder(o['Klientas'], orderNo), // bandom perrašyti paskyros pavadinimą ant lipduko
    address: {
  ...
  var receiver = {
    // Gavėjas (tu) + užsakymo nr — kad ir jei siuntėjo info neatspausdintų, matytum, kuris užsakymas.
 ...
 * (naujas tracking kodas + naujas lipdukas), naudodama tą patį priskirtą pastomatą
 * ir kliento duomenis. Sąmoningai APEINA dublikatų apsaugą (processOrder ją blokuotų).
 *
 * Naudojimas: klientas suklydo paštomatą/telefoną (jau pataisyta lentelėje),
 * siunta pamesta, arba reikia pakartotinės siuntos.
 *
...
function uiCreateNewShipment(orderName) {
  ...
  // Tas pats priskirtas pastomatas (pagal ID). Jei reikia kito — pirma „Pastomatas" mygtuku.
  ...
  // Omniva reikalauja unikalaus partnerShipmentId — pridedam trumpą sufiksą,
  // kad nauja siunta nesikirstų su senąja (jis matysis ir ant lipduko po užsakymo nr).
  var omnivaOrder = rowToOmnivaOrder(o);
  omnivaOrder.partner_shipment_id = String(o['Užsakymas'] || orderName) + '-' + String(Date.now()).slice(-4);

  // 1) Nauja Omniva siunta -> naujas barcode.
  var ship = registerShipment(omnivaOrder, locker);
  var barcode = ship.barcode;

  // 2) Naujas lipdukas į Drive.
  var labelUrl = '';
  try { labelUrl = generateAndStoreLabel(orderName, barcode, cfg('LABEL_TO_EMAIL') || null); } catch (e) {}

  // 3) Perrašom lentelę nauju kodu/lipduku (senasis paminimas pastaboje).
  upsertOrder({
  ...
  // 4) Shopify (tik LIVE): perrašom info + atnaujinam/įvykdom tracking nauju kodu.
